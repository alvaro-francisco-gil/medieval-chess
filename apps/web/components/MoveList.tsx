...
export default function MoveList({ moves, title = "Moves", emptyMessage = "No moves yet. Click a piece to start." }: MoveListProps) {
  ...
  return (
    ...
    >
      ...
      {pairs.length === 0 ? (
        <p className="text-sm" style={{ color: "var(--color-ink-light)" }}>{emptyMessage}</p>
      ) : (
        <div className="space-y-1">
          {pairs.map((pair) => (
            <div key={pair.number} className="flex text-sm font-mono">
              <span className="w-8 text-right mr-2 shrink-0" style={{ color: "var(--color-ink-light)" }}>{pair.number}.</span>
              <span className="w-16" style={{ color: "var(--color-ink)" }}>{pair.white}</span>
              <span style={{ color: "var(--color-ink)" }}>{pair.black ?? ""}</span>
            </div>
          ))}
        </div>
      )}
